// Netlify scheduled function — runs every Sunday (see netlify.toml). Mirrors
// src/lib/runWeeklyReview.js exactly (goal-awareness, sleep correlation,
// self-calibrating step size, progressive overload) but runs with the
// service-role key across every user, since a cron job has no "current
// user" session.
import { createClient } from "@supabase/supabase-js";
import { evaluateWeek, checkDataSufficiency, buildEvidence, buildInsights, calibrateCalorieStep } from "../../src/lib/decisionEngine.js";
...
export async function handler() {
  ...
  const { data: profiles } = await supabase.from("profiles").select("id, deload_week, goal, coach_calorie_step").eq("onboarded", true);

  for (const profile of profiles ?? []) {
    const userId = profile.id;
    const [{ data: metrics }, { data: targets }, { data: sessions }, { data: meals }, { data: thisWeekSets }, { data: priorWeekSets }, { data: recovery }, { data: priorRecovery }, { data: priorReview }, { data: recalibrationReview }] = await Promise.all([
      ...
      supabase.from("recovery_logs").select("*").eq("user_id", userId).gte("log_date", priorWeekStartStr).lt("log_date", weekStartStr),
      ...
      supabase.from("weekly_reviews").select("*").eq("user_id", userId).eq("user_response", "accepted").neq("recommended_calorie_change", 0).order("week_start", { ascending: false }).limit(1).maybeSingle(),
    ]);
    ...
    if (recalibrationReview) {
      ...
      if (didRecalibrate) {
        calorieStep = newStep;
        recalibrated = true;
        await supabase.from("profiles").update({ coach_calorie_step: newStep }).eq("id", userId);
      }
    }
